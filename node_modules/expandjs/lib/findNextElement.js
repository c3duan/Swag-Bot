...
/**
 * Iterates over `node`'s next siblings, returning the first node `identity` returns truthy for.
 ...
 *     // => <li class="item one disabled"></li>
 *
 *     XP.findNextElement(el, '.item');
 *     // => <li class="item two active"></li>
 *
 *     XP.findNextElement(el, '.item', '.disabled');
 *     // => <li class="item five disabled"></li>
 * </script>
 * ```
 *
 ...
 * @since 1.0.0
 * @category dom
 * @description Iterates over `node`'s next siblings, returning the first node `identity` returns truthy for
 * @source https://github.com/expandjs/expandjs/blog/master/lib/findNextElement.js
 *
 * @param {Node} node The target node
 * @param {Element | Function | string} identity The identity of the node to find
 * @returns {Element} Returns the found sibling
 */
module.exports = function findNextElement(node, identity) {
    ...
    // Returning
    return getAllNext(node.parentNode.children, node).find(identity);
};
