...
/**
 * Iterates over `node`'s children, returning the first child `identity` returns truthy for.
 ...
 *     // => <ul id="list">...</ul>
 *
 *     XP.findElement(el, '.item');
 *     // => <li class="item one disabled"></li>
 *
 *     XP.findElement(el, '.item', '.disabled');
 ...
 * @since 1.0.0
 * @category dom
 * @description Iterates over `node`'s children, returning the first child `identity` returns truthy for
 * @source https://github.com/expandjs/expandjs/blog/master/lib/findElement.js
 *
 * @param {Node} node The target node
 * @param {Element | Function | string} identity The identity of the node to find
 * @returns {Element} Returns the first found child
 */
module.exports = function findElement(node, identity) {
    ...
    // Returning
    return getNodes(node).find(identity);
};
