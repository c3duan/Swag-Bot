...
/**
 * Iterates over `node`'s previous siblings, returning the first node `identity` returns truthy for.
 ...
 * @since 1.0.0
 * @category dom
 * @description Iterates over `node`'s previous siblings, returning the first node `identity` returns truthy for
...
module.exports = function findPreviousElement(node, identity) {
    ...
    // Returning
    return getAllPrevious(node.parentNode.children, node).reverse().find(identity);
};
